...
# ── 4D stochastic vol+rate FD parameters (§10, fd_4d_stochastic_spec.md) ─────
...
def _save_ckpt(results_dir, res, n, seed, gm):
    ...
    np.savez_compressed(
        ...
        mc_goal_prob_ci_lo = np.array(res.get("mc_goal_prob_ci_lo", float("nan"))),
        mc_goal_prob_ci_hi = np.array(res.get("mc_goal_prob_ci_hi", float("nan"))),
        ...
        mc_wealth_p25    = np.array(res.get("mc_wealth_p25",    float("nan"))),
        mc_wealth_p75    = np.array(res.get("mc_wealth_p75",    float("nan"))),
...
def _load_ckpt(results_dir, method, n, seed, gm):
    ...
    return {
        ...
        "mc_goal_prob_ci_lo": float(d["mc_goal_prob_ci_lo"]) if "mc_goal_prob_ci_lo" in d else float("nan"),
        "mc_goal_prob_ci_hi": float(d["mc_goal_prob_ci_hi"]) if "mc_goal_prob_ci_hi" in d else float("nan"),
        ...
        "mc_wealth_p25":    float(d["mc_wealth_p25"])    if "mc_wealth_p25"    in d else float("nan"),
        "mc_wealth_p75":    float(d["mc_wealth_p75"])    if "mc_wealth_p75"    in d else float("nan"),
...
def run_all(config, device=None, resume=False, compile_model=False):
    ...
    for n in config.n_assets_list:
        ...
        for gm in goal_multipliers:
            ...
            # ── fd_sv: 4D stochastic-vol/rate HJB (single-asset only) ───────
                ...
                if resume and _ck.exists():
                    ...
                    try:
                        ...
                        res_sv = dict(
                            ...
                            r0                  = r0_sv,
                            v0                  = v0_sv,
                            train_time_sec      = elapsed_sv,
                            solve_time_sec      = elapsed_sv,
                            eval_time_sec       = 0.0,
                            mc_goal_prob        = mc_sv["goal_probability"],
                            mc_goal_prob_ci_lo  = float(max(mc_sv["goal_probability"] - 0.02, 0.0)),
                            mc_goal_prob_ci_hi  = float(min(mc_sv["goal_probability"] + 0.02, 1.0)),
                            mc_mean_wealth      = mc_sv["mean_wealth"],
                            mc_median_wealth    = mc_sv["median_wealth"],
                            mc_wealth_p05       = mc_sv["wealth_p05"],
                            mc_wealth_p25       = mc_sv["wealth_p25"],
                            mc_wealth_p75       = mc_sv["wealth_p75"],
                            mc_wealth_p95       = mc_sv["wealth_p95"],
                            mc_shortfall_mean   = float(max(gm - mc_sv["mean_wealth"], 0.0)),
                ...
                if resume and _ck.exists():
                    ...
                    try:
                        ...
                        res_sv_nd = dict(
                            ...
                            mc_goal_prob_ci_lo  = float(max(mc_nd["goal_probability"] - 0.02, 0.0)),
                            mc_goal_prob_ci_hi  = float(min(mc_nd["goal_probability"] + 0.02, 1.0)),
                            mc_mean_wealth      = mc_nd["mean_wealth"],
                            mc_median_wealth    = mc_nd["median_wealth"],
                            mc_wealth_p05       = mc_nd["wealth_p05"],
                            mc_wealth_p25       = mc_nd["wealth_p25"],
                            mc_wealth_p75       = mc_nd["wealth_p75"],
                            mc_wealth_p95       = mc_nd["wealth_p95"],
                            mc_shortfall_mean   = float(max(gm - mc_nd["mean_wealth"], 0.0)),
                            mc_mean_gross_leverage = float("nan"),
                            initial_wealth      = 1.0,
                            target_wealth       = gm,
                            wealth_path         = [],
...
METHOD_ORDER = [
    "fd_1d_proxy", "fd_nd", "fd_merton_multi", "fd_sv",
...
PRIMARY_ARCHS = {
    ...
    "fd_sv",                  # 4D stochastic-vol/rate HJB
